...
export default function SpareRequests() {
  ...
  const fetchRequests = async () => {
    try {
      const snap = await getDocs(collection(db, 'inventory_spare_requests'))
      const data = []
  ...
  const handleAction = async (id, status) => {
    ...
    try {
      await updateDoc(doc(db, 'inventory_spare_requests', id), {
        status, reviewedBy: user.uid,
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Spare / Reorder Requests</h2>
          <p className="text-slate-500 text-sm">{filtered.length} requests</p>
        </div>
        <button onClick={() => navigate('/services/spare/new')}
                    ...
                    <td className="px-4 py-3">
                      {(!r.status || r.status === 'pending') && (
                        <div className="flex gap-1">
                          <button onClick={() => handleAction(r.id, 'approved')} disabled={actionLoading === r.id + 'approved'}
                            className="px-2 py-1 bg-green-600 hover:bg-green-700 text-white text-xs rounded-lg disabled:opacity-50">✓ Approve</button>
                          <button onClick={() => handleAction(r.id, 'rejected')} disabled={actionLoading === r.id + 'rejected'}
                            className="px-2 py-1 bg-red-600 hover:bg-red-700 text-white text-xs rounded-lg disabled:opacity-50">✗ Reject</button>
                        </div>
                      )}
                      {r.status && r.status !== 'pending' && <span className="text-xs text-slate-400">By {r.reviewedByName}</span>}
                    </td>
                  )}
                </tr>
              ))}

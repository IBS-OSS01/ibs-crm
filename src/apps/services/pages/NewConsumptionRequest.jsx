...
export default function NewConsumptionRequest() {
  const { user, userProfile } = useAuth()
  const navigate = useNavigate()
  const [warehouses, setWarehouses] = useState([])
  const [items, setItems] = useState([])
  const [loading, setLoading] = useState(false)
  const [form, setForm] = useState({ itemName: '', sku: '', qty: 1, locationId: '', reason: '' })
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      await addDoc(collection(db, 'inventory_consumption_requests'), {
      ...
      setSuccess(true)
      setTimeout(() => navigate('/services/consumption'), 1500)
  ...
  return (
    <div className="p-6 max-w-2xl">
      <button onClick={() => navigate('/services/consumption')} className="text-blue-600 text-sm mb-4">← Back</button>
      <h2 className="text-2xl font-bold text-slate-900 tracking-tight mb-6">New Consumption Request</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input type="text" value={form.itemName} onChange={e => setForm(p => ({ ...p, itemName: e.target.value }))}
              placeholder="e.g. Conveyor Belt Roller" autoComplete="off"
          ...
          <div>
            ...
            <input type="text" value={form.sku} onChange={e => setForm(p => ({ ...p, sku: e.target.value }))}
              placeholder="e.g. CB-ROL-001" autoComplete="off"
          ...
          <div>
            ...
            <textarea value={form.reason} onChange={e => setForm(p => ({ ...p, reason: e.target.value }))}
              placeholder="Why is this needed?" rows={3}
          ...
          <div className="flex gap-3">
            ...
            <button type="button" onClick={() => navigate('/services/consumption')}
